...
class ContactListController extends Controller
{
    ...
    public function index()
    {
        ...
        // $contactList = ContactList::where('status', 'confirm')->where('office_id', $office_id)
        //     ->orWhere('contact_id', $office_id)->paginate(10);
    ...
    public function search(Request $request)
    {
        ...
        $ids=[];
        $office_id = auth()->user()->office_id;
        $officeName= $request->search;
        // $contactList = ContactList::where('status', 'confirm')->where('office_id', $office_id)
        //     ->orWhere('contact_id', $office_id)->paginate(10);
             $contactList =  ContactList::where('status', 'confirm')->where(function ($query) use ($office_id) {
            $query->where('office_id', $office_id)->orWhere('contact_id', $office_id);
        })->whereHas('Offices', function ($query) use ($officeName) {
            $query->where('office_name', 'like', '%' . $officeName . '%');
        })->whereHas('contactOffices', function ($query) use ($officeName) {
            $query->where('office_name', 'like', '%' . $officeName . '%');
        })->get();
    ...
    public function store(ContactListStoreRequest $request)
    {
        ...
        if ($check) {
            return response()->json(['failed' => 'Contact List Already Exists', 'data' => $check], 405);
        } else {
            ...
            $notification['data'] = ContactList::with(['Offices'])->where('office_id',$contactList->office_id)->where('contact_id',  $contactList->contact_id)->first();
            ...
            switch ($request->message_type) {
                case 'text':
                    ...
                    $message = Message::create([
                        ...
                        'received_office_id' => $request->receiver_id,
                    ]);
                    ...
                    $message->load('sender','senderOffice','receiverOffice');
    ...
    public function messages(int $id)
    {
        ...
        if ($contactList) {
            $messages = Message::with(['sender','senderOffice','receiverOffice'])->where('contact_id', $contactList->id)->latest()->paginate(10);
