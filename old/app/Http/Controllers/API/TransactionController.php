...
class TransactionController extends Controller
{
    ...
    public function index()
    {
        ...
        $transactions = Transaction::where('sender_id', $office_id)->orWhere('receiver_id', $office_id)
    ...
    public function office(string $id)
    {
        // المستلمه و المرسله
        $office_id = auth()->user()->office_id;
        $transactions = Transaction::where(function ($query) use ($office_id, $id) {
        ...
        if($transactions){
            ...
            return response()->json(['failed' => 'transactions not found ', 'data' => $transactions], 404);
        }
    }
    ...
    public function Manualoffice(string $id)
    {
        // المستلمه و المرسله
        $office_id = auth()->user()->office_id;
        $transactions = Transaction::where('office_type','manual')->where('sender_id',$office_id)->where('receiver_id', $id)->with(['user', 'ManualReciver', 'Sender', 'currency'])->get();
        if($transactions){
            foreach ($transactions as $transaction) {
                $transaction->currency->currencyName = $transaction->currency->currency->name;
                unset($transaction->currency->currency);
    ...
    public function confirmRecived(Request $request, $id)
    {
        ...
        if ($transactions) {
            ...
            return response()->json(['success' => 'transaction requests retrived succefull'], 200);
        } else {
