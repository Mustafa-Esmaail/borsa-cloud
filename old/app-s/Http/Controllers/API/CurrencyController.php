...
class CurrencyController extends Controller
{
    ...
    public function offices(){
        ...
        foreach($currencies as $cur){
            $data['id']=$cur->currency_id;
            $data['name']=$cur->currency->name;
            $data['country']=$cur->currency->country;
            $data['sell_price']=$cur->sell_price;
            $data['buy_price']=$cur->buy_price;
            $data['wallet_balance']=$cur->wallet_balance;
        }

        return response()->json(['success' => 'Currencies retrived succefull','currencies'=>$data], 200);
    }
